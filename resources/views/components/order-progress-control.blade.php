{{-- Order Progress & Deadline Control Component --}}
@props(['order'])
...
@if($canManage && in_array($order->status, ['paid', 'processing', 'waiting_confirmation', 'needs_revision']))
<div class="glass p-4 sm:p-6 rounded-lg mb-6 border-2 border-primary/30" x-data="orderControls">
        ...
        <div>
            ...
            <div class="glass p-3 rounded-lg mb-3">
                ...
                <p class="font-semibold text-sm flex items-center gap-2
                    {{ $order->deadline_at->isPast() ? 'text-red-400' : ($order->deadline_at->diffInHours() < 24 ? 'text-yellow-400' : 'text-white') }}">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                    {{ $order->deadline_at->format('d M Y, H:i') }}
                    <span class="text-xs">({{ $order->deadline_at->diffForHumans() }})</span>
                </p>
            </div>
            @endif
    ...
    @if($order->type === 'service' && in_array($order->status, ['processing', 'waiting_confirmation', 'needs_revision', 'paid']))
    <div class="mt-6 pt-6 border-t border-white/10 {{ in_array($order->status, ['waiting_confirmation', 'needs_revision']) ? 'bg-orange-500/5 p-4 rounded-lg border-orange-500/30' : '' }}">
...
document.addEventListener('alpine:init', () => {
    Alpine.data('orderControls', () => ({
        ...
        deadline: '{{ $order->deadline_at ? $order->deadline_at->format('Y-m-d\TH:i') : '' }}',
        saving: false,
        ...
        async saveProgress() {
            ...
            try {
                const response = await fetch('{{ route('orders.updateProgress', $order) }}', {
                    method: 'POST',
                    headers: {
        ...
        async saveDeadline() {
            ...
            try {
                const response = await fetch('{{ route('orders.setDeadline', $order) }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
