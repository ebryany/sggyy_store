    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-3.5 sm:py-4 lg:py-5">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center">
                <a href="{{ route('home') }}" class="flex items-center gap-2 sm:gap-3 hover:opacity-80 transition-all duration-300">
                    @php
                    ...
                    @if($logoUrl)
                        ...
                        <span style="display: none;" class="text-xl sm:text-2xl lg:text-3xl font-bold text-primary">{{ substr($siteName, 0, 1) }}</span>
                    @endif
                    @if(!$logoUrl)
                        <span class="text-xl sm:text-2xl lg:text-3xl font-bold text-primary glow-sm">{{ $siteName }}</span>
                    @endif
                </a>
            </div>
            ...
            <div class="flex items-center gap-3 sm:gap-4">
                ...
                <a href="{{ route('cart.index') }}" 
                   ...
                   x-data="{ 
                    ...
                    <x-icon name="shopping-cart" class="w-6 h-6 text-white" />
                ...
                @auth
                    ...
                    <div x-data="{ open: false }" class="relative">
                        ...
                        <div x-show="open" 
                            ...
                            <a href="{{ route('admin.dashboard') }}" class="block px-4 py-3 hover:bg-white/10 touch-target text-sm sm:text-base bg-primary/10 border-l-4 border-primary flex items-center gap-2">
                                <x-icon name="target" class="w-4 h-4" />
                                Admin Dashboard
                            </a>
                            @else
                            {{-- User biasa ke Dashboard User --}}
                            <a href="{{ route('dashboard') }}" class="block px-4 py-3 hover:bg-white/10 touch-target text-sm sm:text-base flex items-center gap-2">
                                <x-icon name="dashboard" class="w-4 h-4" />
                                Dashboard
                            </a>
                            @endif
                            {{-- Dashboard Seller: Jika sudah verified seller → dashboard, jika belum → verification --}}
                            @if(auth()->user()->isVerifiedSeller())
                            <a href="{{ route('seller.dashboard') }}" class="block px-4 py-3 hover:bg-white/10 touch-target text-sm sm:text-base flex items-center gap-2">
                                <x-icon name="chart" class="w-4 h-4" />
                                Dashboard Seller
                            </a>
                            @else
                            <a href="{{ route('seller.verification.index') }}" class="block px-4 py-3 hover:bg-white/10 touch-target text-sm sm:text-base flex items-center gap-2">
                                <x-icon name="shield" class="w-4 h-4" />
                                Jadi Seller
                            </a>
                            @endif
                            <a href="{{ route('profile.index') }}" class="block px-4 py-3 hover:bg-white/10 touch-target text-sm sm:text-base flex items-center gap-2">
                                <x-icon name="user" class="w-4 h-4" />
                                Profile
                            </a>
                            <a href="{{ route('wallet.index') }}" class="block px-4 py-3 hover:bg-white/10 touch-target text-sm sm:text-base flex items-center gap-2">
                                <x-icon name="wallet" class="w-4 h-4" />
                                Wallet
                            </a>
                            <a href="{{ route('orders.index') }}" class="block px-4 py-3 hover:bg-white/10 touch-target text-sm sm:text-base flex items-center gap-2">
