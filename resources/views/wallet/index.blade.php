@extends(request()->routeIs('seller.*') ? 'seller.layouts.dashboard' : 'layouts.app')

@section('title', 'Wallet - Ebrystoree')
...
<div class="container mx-auto px-3 sm:px-4 py-6 sm:py-8 lg:py-12">
    ...
    <!-- Filters -->
    <div class="glass p-4 sm:p-6 rounded-lg mb-4 sm:mb-6" x-data="{ type: '{{ request('type') ?? '' }}', status: '{{ request('status') ?? '' }}' }">
        <div class="flex items-center gap-2 mb-4">
            <x-icon name="filter" class="w-5 h-5 text-primary" />
            <h3 class="text-base sm:text-lg font-semibold">Filter Transaksi</h3>
        </div>
        <form method="GET" action="{{ route('wallet.index') }}" class="flex flex-col sm:flex-row gap-3 sm:gap-4">
            <select name="type" 
                    x-model="type" 
