...
@section('title', 'Jasa Joki - Ebrystoree')
...
<div class="container mx-auto px-3 sm:px-4 py-6 sm:py-8 lg:py-12">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 sm:mb-8">
        ...
        @auth
            ...
            <a href="{{ route('seller.services.create') }}" class="w-full sm:w-auto px-4 sm:px-6 py-3 bg-primary hover:bg-primary-dark rounded-lg transition-colors text-sm sm:text-base text-center touch-target">
                + Tambah Jasa
            </a>
            @endif
        @endauth
    ...
    <div class="glass p-3 sm:p-4 rounded-lg mb-4 sm:mb-6">
        <form method="GET" action="{{ route('services.index') }}" class="space-y-4">
            ...
            <div class="flex flex-col sm:flex-row gap-3 sm:gap-4">
                ...
                {{-- Category filter removed - Services don't have category column --}}
            </div>
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                <div class="flex flex-wrap gap-3">
                    ...
                    @if(request()->anyFilled(['search', 'category', 'min_price', 'max_price', 'rating', 'sort', 'seller', 'duration_min', 'duration_max', 'per_page']))
                    <a href="{{ route('services.index') }}" class="px-4 sm:px-6 py-2 glass glass-hover rounded-lg text-center text-base sm:text-sm touch-target">
                        Reset
                    </a>
                    @endif
                </div>
