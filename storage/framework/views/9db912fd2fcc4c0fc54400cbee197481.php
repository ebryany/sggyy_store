<?php $__env->startSection('title', 'Jasa Joki - Ebrystoree'); ?>

<?php $__env->startSection('content'); ?>
<div class="container mx-auto px-3 sm:px-4 py-6 sm:py-8 lg:py-12">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 sm:mb-8">
        <h1 class="text-2xl sm:text-3xl lg:text-4xl font-bold">Jasa Joki</h1>
        <?php if(auth()->guard()->check()): ?>
            <?php if(auth()->user()->isSeller() || auth()->user()->isAdmin()): ?>
            <a href="<?php echo e(route('seller.services.create')); ?>" class="w-full sm:w-auto px-4 sm:px-6 py-3 bg-primary hover:bg-primary-dark rounded-lg transition-colors text-sm sm:text-base text-center touch-target">
                + Tambah Jasa
            </a>
            <?php endif; ?>
        <?php endif; ?>
    ...
    <div class="glass p-3 sm:p-4 rounded-lg mb-4 sm:mb-6">
        <form method="GET" action="<?php echo e(route('services.index')); ?>" class="space-y-4">
            <!-- Row 1: Search -->
            ...
            <!-- Row 2: Price Range, Rating, Duration & Sort -->
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
                ...
                <div>
                    <label class="block text-xs text-white/60 mb-1">Durasi (Jam)</label>
                    <input type="number" 
                           name="duration_max" 
                           value="<?php echo e(request('duration_max')); ?>" 
                           placeholder="Max durasi" 
                           min="0"
                           class="w-full glass border border-white/10 rounded-lg px-4 py-2 bg-white/5 focus:outline-none focus:border-primary text-base sm:text-sm touch-target">
                </div>
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                <div class="flex flex-wrap gap-3">
                    ...
                    <?php if(request()->anyFilled(['search', 'category', 'min_price', 'max_price', 'rating', 'sort', 'seller', 'duration_min', 'duration_max', 'per_page'])): ?>
                    <a href="<?php echo e(route('services.index')); ?>" class="px-4 sm:px-6 py-2 glass glass-hover rounded-lg text-center text-base sm:text-sm touch-target">
                        Reset
                    </a>
                    <?php endif; ?>
                </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
        ...
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
            <div class="col-span-full text-center py-12">
                <p class="text-white/60 text-lg mb-4">Belum ada jasa tersedia.</p>
                <?php if(auth()->guard()->check()): ?>
                    <?php if(auth()->user()->isSeller() || auth()->user()->isAdmin()): ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\febry\Documents\my_jasa\resources\views/services/index.blade.php ENDPATH**/ ?>