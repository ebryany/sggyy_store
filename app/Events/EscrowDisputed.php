...
class EscrowDisputed implements ShouldBroadcast
{
    use Dispatchable, InteractsWithSockets, SerializesModels;

    public Escrow $escrow;
    public Order $order;
    public int $disputedBy;

    public function __construct(Escrow $escrow, Order $order, int $disputedBy)
    {
        $this->escrow = $escrow;
        $this->order = $order;
    ...
    public function broadcastWith(): array
    {
        return [
            ...
            'dispute_reason' => $this->escrow->dispute_reason,
            'disputed_at' => $this->escrow->disputed_at?->toIso8601String(),
            'disputed_by' => $this->disputedBy,
        ];
    }
}
